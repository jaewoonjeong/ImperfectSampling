...
iteration=1
samplesize=c(6,10,14,20,30,40) # number of fish to be sampled
...
par(oma=c(1,2,1,1))
filled.contour(cex=0.4,x=samplesize,y=SI,z=ct[1,,],ylim=c(1,22),zlim=c(-0.51,0.4),color=terrain.colors,xlab="Sample Size",
               ylab="Sampling Interval (days)",key.title=title(main=""),main="",cex.lab=1.5,
               plot.axes = { axis(1, seq(5, 40, by = 5))
                 axis(2, seq(1, 22, by = 3)) })
