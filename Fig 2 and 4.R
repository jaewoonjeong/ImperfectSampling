...
par(mfcol=c(3,1),mar=c(0.3,5,1,1),oma=c(6,1,1,2),cex.lab=1.2)
colfunc <- colorRampPalette(c("black", "gray"))
matplot(samplesize,t(TxFreq[c(1,2,3,7),]),cex.main=1.5,cex.lab=1.5,cex.axis=1.5,type="l",pch=1,las=1,main="",xlab="",lwd=3,ylab="Frequency of Treatments",lty=1,col=colfunc(4),ylim=c(5.5,7.8),axes=F); axis(2,las=1); box() #axis(1,seq(10,100,10),seq(10,100,10)); 
mtext('(A)',side=3,line=-2,at=109,cex=1.2,font=2)
legend("topright",legend=TT[c(1,2,3,7)],title="Treatment Threshold",col=colfunc(4),lty=1,lwd=3,cex=1,horiz = T)
matplot(samplesize,t(mIB[c(1,2,3,7),]),type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5,pch=1,las=1,xlab="",main="",ylab="IB",col=colfunc(4),ylim=c(10,600),lty=1,log='y',lwd=3,axes=F); axis(2,las=1); box() #axis(1,seq(10,100,10),seq(10,100,10)); 
mtext('(B)',side=3,line=-2,at=109,cex=1.2,font=2)
matplot(samplesize,t(mIBOT[c(1,2,3,7),]),type="l",cex.main=1.5,cex.lab=1.5,cex.axis=1.5,pch=2,las=1,xlab="Sample Size",main="",ylab="IB Over Threshold",col=colfunc(4),lty=1,ylim=c(1,100),axes=F,log='y',lwd=3); axis(1,seq(10,100,10),seq(10,100,10)); axis(2,las=1); box()
mtext("Sample Size",side=1,line=3); mtext('(C)',side=3,line=-2,at=109,cex=1.2,font=2)
