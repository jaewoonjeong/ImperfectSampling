...
for(i in 2633){for(l in c(3)){for(s in c(2)){ #
...
p0.2<-ggplot(subset(diff,Threshold==0.2&SampleSize!=50),aes(x=Difference,y=SampleSize,group=SampleSize))+
  geom_density_ridges(alpha=0.7)+xlim(-1.2,1.2)+xlab("(Monitored Abundance) - (True Abundance)")+ylab("")+
  geom_vline(xintercept = 0,linetype="dashed",color="grey40",size=1)
p1<-ggplot(subset(diff,Threshold==1&SampleSize!=50),aes(x=Difference,y=SampleSize))+geom_density_ridges()+xlim(-1.2,1.2)+xlab("(Monitored Abundance) - (True Abundance)")+ylab("")+
  geom_vline(xintercept = 0,linetype="dashed",color="grey40",size=1)
p3<-ggplot(subset(diff,Threshold==3&SampleSize!=50),aes(x=Difference,y=SampleSize))+geom_density_ridges()+xlim(-1.2,1.2)+xlab("(Monitored Abundance) - (True Abundance)")+ylab("")+
  geom_vline(xintercept = 0,linetype="dashed",color="grey40",size=1)
ggarrange(p0.2,p3,ncol=1)

###################### Figure 5
ee=matrix(NA,length(TT)*length(samplesize)*iteration*4); dim(ee)=c(length(TT),length(samplesize),iteration,4) 
for(l in 1:length(TT)){for(s in 1:length(samplesize)){for(i in 1:iteration){
  ee[l,s,i,]=c(length(which(na.omit(Error[,l,s,i])=="C")),length(which(na.omit(Error[,l,s,i])=="E")),length(which(na.omit(Error[,l,s,i])=="L")),length(which(na.omit(Error[,l,s,i])=="M")))}}}
SumEE=matrix(NA,length(TT)*length(samplesize)*iteration); dim(SumEE)=c(length(TT),length(samplesize),iteration)
for(l in 1:length(TT)){for(s in 1:length(samplesize)){for(i in 1:iteration){SumEE[l,s,i]=sum(ee[l,s,i,])}}}
mmm=matrix(NA,length(TT),length(samplesize)); for(l in 1:length(TT)){for(s in 1:length(samplesize)){mmm[l,s]=mean(SumEE[l,s,])}}
matplot(x=samplesize, y=t(mmm),xlab="Sample Size",ylab="Frequency of Treatments",col=5:8,las=1,type="o",pch=1,ylim=c(5.5,8))
legend("topright",legend=TT,title="Treatment Threshold",col=5:8,lty=1:4,pch=1)
...
boxplot(list(No[,7]/No[1,7],E1[,7]/No[1,7],E2[,7]/No[1,7],E3[,7]/No[1,7],B1[,7]/No[1,7],B2[,7]/No[1,7],B3[,7]/No[1,7],
             L1M1[,7]/No[1,7],L2M2[,7]/No[1,7],L3M3[,7]/No[1,7],L1M2[,7]/No[1,7],L2M3[,7]/No[1,7],L3M4[,7]/No[1,7]),main="",xlab="",ylab="",las=1,axes=F)
abline(h=1,lty=2); axis(2,las=1); box(); mtext("Relative IB",side=2,line = 3); abline(v=c(1.5,4.5,7.5,10.5),lty=3,col="grey")
boxplot(list(No[,8]/No[1,8],E1[,8]/No[1,8],E2[,8]/No[1,8],E3[,8]/No[1,8],B1[,8]/No[1,8],B2[,8]/No[1,8],B3[,8]/No[1,8],
             L1M1[,8]/No[1,8],L2M2[,8]/No[1,8],L3M3[,8]/No[1,8],L1M2[,8]/No[1,8],L2M3[,8]/No[1,8],L3M4[,8]/No[1,8]),main="",xlab="",ylab="",las=1,axes=F,ylim=c(0,4.5))
